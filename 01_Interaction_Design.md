[original](http://www.interaction-design.org/encyclopedia/interaction_design.html)
#1.インタラクションデザイン
...
インターネットの浸透とあわせて、家庭での娯楽のためのコンピュータ利用がはじまり、デジタルでインタラクティブなコンシューマプロダクトが出現しました。そこで、デザインとエンジニアリングの二つのカルチャーが、任意の意味でユーザーエクスペリエンスに引きつけられていきました。世紀の変わり目頃にかけて、インタラクションデザインは```ちょっと難しいので後回し中```
...
この頃から、たくさんのプロによる実践や、アカデミックなプログラが行われ、記事が書かれ、交流や会議がインタラクションデザイン、という言葉の元に形成されるようになりました。言うまでも無く、インタラクションデザインという言葉の意味は多様に理解されています。これらの定義がどの程度多岐にわたっているかを知るすべはありませんが、その代わりに、私はインタラクションデザインの簡単な体系化を行おうと思います。それは、先述したインタラクションデザインという言葉の意味を引き継ぎ、かつ、この領域の限界線を描くような物です。以下の様な感じです。
...
インタラクションデザインとは、人が使うためのデジタルな物を形作ることである

これは実にシンプルな表現です。しかし各個の要素について述べる段に至ると、これでは全く説明力不足であることが明らかになってきます。
...
"shaping" 形作るという言葉は、デザイナー的な活動を意味するよう意識して使っています。（一方でたとえば "building" はエンジニアリング、“making”や“creating”はおよそどう言った対象に対しても使うことにします） より具体的にはこの言葉は、構想するというのがデザインのプロセスであると言う点において、インタラクションデザイン特有の表現であると思います。この言葉は5つの主要な特徴を含んでいます。``` capital-Dの下りは分からないので後回し```

###1.1.1 デザインは物質を形作り配置する事で、状況を変化させる
...
言い換えると、デザインとは変化のことであり、デザインする物体こそが究極には、デザイナーにとってシチュエーションの変化を始めるための手段である、と言うことです。
...
このことはインタラクションデザイナーが何をデザインするのか、ということと関わってきます。私は、インタラクションデザイナーはデジタルな物を扱う、と言う制約をかけることを提案しています。しかし今のところそれが意味するとことは、```ちょっとむずかしい…```

###1.1.2 デザインとは将来の可能性を探索すること
> 1.1.2 Design is about exploring possible futures

> This seems almost too obvious to point out, but from an academic point of view it might be worth mentioning since it entails a fundamental difference in orientation; analytical and critical studies focus on that which exists, whereas design concerns itself with that which could be. This has epistemological ｓconsequences for, e.g., how research is conducted. 

> Framing design as exploration also means that it often makes sense to spend time in early phases on divergent work, essentially looking around in a design space of possibilities before committing to a particular direction. Exploring possible futures in interaction design often involves inviting the future users in various forms of participation.

これはあまりに明らかなことのようですが、学術的な視点からすると全く異なる方向性を伴うため、触れる必要のあることです。 分析し批判する研究は、対象が存在するところにフォーカスしますが、デザインはそれ自身が、なり得る物取り扱います。```epistemologicalが分からないため飛ばしています```

探索としての枠組みのデザインは、同時に、発散する作業の初期の段階で、ある特定の方向に注力する以前に、デザインする領域の可能性を見渡す上で非常に意味を持ちます。インタラクションデザインにおいて可能性のある未来を探索することとは、将来のユーザーに様々な形で参加してもらうこと、などを含みます。
...
デザインには将来の可能性を探索することが不可欠であると主張することは、ユーザスタディやそれらの要約的評価はインタラクションデザインを構成しない、ということを意味します。しかし、それらはしばしばインタラクションデザインのプロセスに含まれているように扱われます。おそらく、フィールドワークや新しい手法の導入、評価をデザインプロセスに含むことが、広い意味で将来の可能性を探索するプロセスに含まれている、と考えた方が良いのでしょう。


###1.1.3 デザインは課題の枠組み作りと並行してあり得るソリューションをつくる
...
シチュエーションを変化させ、将来の可能性を探索するという見解は、我々が何かをデザインする際、それが利用されるシチュエーションは、既に元のシチュエーションとは異なる、と言う結論を導き出します。これは打って変わって、デザインによって解決すべき既存の問題を分析し定義することは、本質的に限られた利点を持っている、と言うことを意味します。将来の可能性を探索することは、様々なデザインソリューションを意味するだけでなく、様々な課題を見つけ出すことでもあります。現在のインタラクションデザインの実践現場においては、永続的なベータ版を開発するようなアプローチ…```難しい…```
...
この特徴のために、クリエイティブなデザインの前段として、求められた詳細に関する記述的な分析をすることを目的として開発やエンジニアリングのプロセスを行うという従来のシステムは、デザイナーのプロセスである、とは考えられなくなっています。これはとても意図的なことであります。

###1.1.4 デザインはスケッチやその他のタンジブルな表現を介して思考される
...
将来の可能性のある側面やスナップショットをスケッチするとき（たとえば、未だ存在しないプロダクトを描くなど）、デザイナーは単に内なる目に写っているイメージをコピーしているだけと言うわけではありません。ドローイングは、手や感覚や思考を駆使してた細かい思考の繰り返しにおいて発生する強弱や変化の可能性に応じて行われる、"micro-experiments" 小さな実験なのです。このことは、デザイン実践における他のスケッチメディアにも適応されます。インタラクションデザインにおいては、現在のデザインマテリアルの特質による影響が見られます。たとえば、革新的なインタラクションデザインを行うとき、ローファイなスケッチ道具にとどまるより、新しいソフトウェアやハードウェアを駆使することが求められます。
...
一般的にスケッチをすると言うことは、どの道具を使うかと言うことより、デザイナーのマインドセットの問題です。未完成のアイディアの意味するところや詳細についての議論にデザイナーが関わるなら、またそれがクイックで一時的で破棄しうる物なら、スケッチが適していると言えます。ナプキンに絵を描くようなレベルから、ちょっとしたプログラミングをするところまで、さらにはもしかすると出荷される商品を作る際に使われるプログラムをするところまでいろいろありますが、肝心なのは目的と意図です。

###1.1.5 デザインは道具、技術、美的な事、そして倫理的な側面を一貫して扱う
...
デザインプロセスにおいて探索される将来の可能性の一つ一つは、すべての局面において、議論とトレードオフを導き出します。そしてそれらに続く明確な道は通常ありません。これはインタラクションデザインにおいても同様です。技術的な決定は最終的なインタラクションの美的な品質に影響し、提供するフィーチャのための道具の選定は、倫理的に反発をうけたりもします。
...
歴史的に見て、HCI, ユーザビリティエンジニアリング、ヒューマンファクタの領域は、道具や技術的な側面に偏りがちです。デザイナー的なアクティビティとしてのインタラクションデザインは、美的、倫理的品質が絶対に無視されたり軽視されてはいけない、と主張しなければなりません。見た目にも使い勝手にも良い物か、またそれが社会的責任やモラルスタンダードの見地においても安心できる物か、それらの基準ががユーザエクスペリエンス全体においてだけで無く、計測可能な物質的成果物においても、重大なインパクトを持ちます。インタラクションデザイナーの方は、ユーザはみな複雑な感覚を持っているので、デザインプロセスはそれらに従う形で執り行われる必要がある、と肝に銘じておきましょう。

##1.2 デジタルマテリアルとインタラクションデザイン
...
デジタルな物は、インタラクションデザインによって形作られます。インタラクションデザイナーはデジタルな素材、ソフトウェア、電子機器、ネットワークなどを使って仕事をしている、と言うことは非常に重要なことです。デジタルな素材を扱うには、たとえば絵を描く練習をすることなどが必要になってきます。イノベーティブなインタラクションテクニックを設計する際、それに頼りになるような過去の経験は無いでしょうから、ソフトウェアやハードウェアを組み合わせる実験が必要とされます。そのような組み合わせは、絵を描くようなマインドセットで行われるでしょうが、```後回し```

> Historically, the digital things made by interaction designers were largely tools � contraptions intended to be used instrumentally, for solving problems and carrying out tasks, and mostly to be used individually. Much of our ingrained best-practice knowledge in the field emanates from this time, expressed in concepts such as user goals, task flows, usability and utility. However, it turns out that digital technology in society today is mostly used for communication, i.e., as a medium. 

歴史的に見て、インタラクションデザイナーによって作られたデジタルな物の多くは、問題を解決しタスクをこなし、個別に利用される道具として作られています。我々に深く染みついているこの領域におけるベストプラクティスな知見は、ユーザーの目的や、仕事の流れや、ユーザビリティや有用性について語っています。しかし、今日の社会のおいては、デジタル技術は多くの場合、コミュニケーションつまり、媒体として使われている事が分かりました。

> And as a medium, it has characteristics that set it apart from previously existing personal and mass communication media. For example, it lowers the thresholds of media production to include virtually anyone, it provides many-to-many communication with persistent records of all exchanges that transpire, and it offers access to ongoing modifications of its infrastructures. These characteristics of what we might call collaborative media are only beginning to be understood in interaction design, and one might expect that this will be one of the most significant areas for future conceptual developments in our field.

媒体としては、従来存在していた個人的またはマスなコミュニケーションとは隔たった性質を持っています。それはたとえば、実質的に誰でも巻き込むことが可能になったため、メディアを作る敷居が下がったり、多対多のコミュニケーションをちゃんと記録を伴って行うことが出来るようになったり、そのインフラそのものをリアルタイムに改善していく為の手段を提供したりしています。我々がコラボラティブメディアと呼んでいるこれらの特性は、インタラクションデザインの界隈では認知され始めたばかりで、これが未来を切り拓く上でもっとも重要な領域だと期待されています。
...
インタラクションデザインをデジタルな物（メディアを含む）に限定する事で、サービスのデザインや組織のデザイン、社会政治的な仲介などの多くのパートについては扱わないことにしています。歴史的に見ると、エンタープライズ向けのシステムコンサルタントが、1980年代に給与支払簿を管理するシステムを求められたのと似ています。現状を調査すると、古いシステムは大きな問題を抱えていると言うわけでは無い事が分かったけど、人事部のワークフローは非常に複雑で不自由な物でした。コンサルタントは新しいシステムを提案すべきでしょうか、それとも組織開発のコンサルタントの必要性を的確に指摘すべきでしょうか。あるいは、組織の仲介を自ら行うべきでしょうか。
